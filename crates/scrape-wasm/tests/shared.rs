//! Cross-platform integration tests for scrape-rs WASM bindings.
//!
//! This module loads shared test cases from `test_cases.json` at compile time and
//! executes them against the WASM implementation. The same test cases are run by
//! Rust, Python, and Node.js test runners to ensure API consistency.

#![cfg(target_arch = "wasm32")]

use std::collections::HashMap;

use scrape_wasm::Soup;
use serde::Deserialize;
use wasm_bindgen_test::*;

wasm_bindgen_test_configure!(run_in_browser);

const TEST_CASES_JSON: &str = include_str!("../../../tests/shared/test_cases.json");
...
fn run_find_assertion(soup: &Soup, selector: &str, expected: &FindExpected, test_id: &str) {
    let result = soup
        .find(selector)
        .unwrap_or_else(|e| panic!("[{test_id}] find('{selector}') failed with error: {e:?}"));

    if expected.exists == Some(false) {
        assert!(result.is_none(), "[{test_id}] Expected selector '{selector}' to find nothing");
        return;
    }
    ...
    let tag = match result {
        ...
        None => {
            if expected.exists != Some(false) {
                panic!("[{test_id}] Expected selector '{selector}' to find element");
            }
            return;
        }
    };
    ...
    if let Some(missing_attr) = &expected.attr_missing {
        assert!(
            ...
            "[{test_id}] Expected attribute '{missing_attr}' to be missing"
        );
    }
    ...
    if let Some(classes) = &expected.has_class {
        for class in classes {
            assert!(tag.has_class(class), "[{test_id}] Expected element to have class '{class}'");
        }
    }
    ...
    if let Some(not_classes) = &expected.not_has_class {
        for class in not_classes {
            assert!(
                !tag.has_class(class),
                "[{test_id}] Expected element to NOT have class '{class}'"
            );
        }
    }
}
...
fn run_find_all_assertion(soup: &Soup, selector: &str, expected: &FindAllExpected, test_id: &str) {
    let results = soup
        .find_all(selector)
        .unwrap_or_else(|e| panic!("[{test_id}] find_all('{selector}') failed with error: {e:?}"));

    if let Some(count) = expected.count {
        assert_eq!(results.len(), count, "[{test_id}] Count mismatch for find_all('{selector}')");
    }
}
...
fn run_find_then_assertion(
    ...
    match chain {
        ...
        "children" => {
            ...
            if let Some(count) = expected.count {
                assert_eq!(children.len(), count, "[{test_id}] Children count mismatch");
            }
        }
        "next_sibling" => {
        ...
        "descendants" => {
            let descendants = tag.descendants();
            if let Some(count) = expected.count {
...
fn run_scoped_find_assertion(
    ...
    let result = scope_tag
        .find(selector)
        .unwrap_or_else(|e| panic!("[{test_id}] scoped find('{selector}') failed: {e:?}"));

    if expected.exists == Some(false) {
        assert!(
            result.is_none(),
            "[{test_id}] Expected scoped selector '{selector}' within '{scope}' to find nothing"
        );
        return;
    }
...
fn run_scoped_find_all_assertion(
    ...
    let results = scope_tag
        .find_all(selector)
        .unwrap_or_else(|e| panic!("[{test_id}] scoped find_all('{selector}') failed: {e:?}"));

    if let Some(count) = expected.count {
        assert_eq!(results.len(), count, "[{test_id}] Scoped find_all count mismatch");
    }
}

fn run_test_case(case: &TestCase) {
    let soup = Soup::new(&case.input, None);
